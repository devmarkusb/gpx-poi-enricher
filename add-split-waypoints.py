...
def point_at_distance(points, cum, target):
    if target <= 0:
...
def add_split_waypoints(input_file, output_file, segments=10):
    with open(input_file, "r", encoding="utf-8") as f:
        gpx = gpxpy.parse(f)

    points = collect_track_points(gpx)
    cum = cumulative_lengths(points)
    total = cum[-1]

    for i in range(1, segments):
        frac = i / segments
        wpt = point_at_distance(points, cum, total * frac)
        wpt.name = f"Split {i}"
