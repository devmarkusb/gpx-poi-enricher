...
Typical usage:
  python add_pois_to_gpx.py split.gpx spielplaetze.gpx --profile "Spielplatz" --max-km 10
  python add_pois_to_gpx.py split.gpx camping.gpx --profile "Campingplatz" --max-km 10
...
USER_AGENT = "gpx-poi-enricher/2.0 (personal route planning)"
...
def sample_track_by_distance(points, spacing_km=12.0):
    sampled = [points[0]]
    dist_since = 0.0
...
def main():
    ...
    ap.add_argument("--sample-km", type=float, default=15.0, help="Track sampling spacing in km (default: 12)")
    ...
    ap.add_argument("--batch-size", type=int, default=6, help="Overpass query batch size (default: 6)")
    ...
    country_segments = detect_country_segments(sampled, session, min_spacing_for_reverse_km=args.country_sample_km)
    ...
    for cc, pts in country_segments.items():
        ...
        for batch in chunked(pts, args.batch_size):
            query = build_query(batch, args.max_km, profile_name, cc)
            data = query_overpass(session, query)
            candidates = extract_candidates(data, track_points, args.max_km, profile_name)
